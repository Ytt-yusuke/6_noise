Shader "Unlit/otherRTShader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float noise =
                    Noise(IN.globalTexcoord * 4 + _Time.y * 0.01) +
                    Noise(IN.globalTexcoord * 8 + _Time.y * 0.04) * 0.5 +
                    Noise(IN.globalTexcoord * 16 + _Time.y * 0.16) * 0.25 +
                    Noise(IN.globalTexcoord * 32 + _Time.y * 0.64) * 0.125;
